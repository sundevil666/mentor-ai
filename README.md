...
The book reader uses viewport-based pages rather than vertical scrolling. A tap on an English word plays it and opens the right-sidebar lookup card. Native press-and-hold selection can expand the lookup to a phrase. Translation uses the authenticated API route backed by Google Cloud Translation Basic; configure the server-only `GOOGLE_TRANSLATE_API_KEY` and enable billing/quota for Cloud Translation. Never expose this key in the PWA build. Single-word IPA is requested separately from Dictionary API because Cloud Translation does not return phonetic transcription.

Successful lookups are retained in `vocabulary-practice-items` locally and synchronized to the private server learning state. Repeated lookups become vocabulary weaknesses and review priorities for mentor lesson planning. The system records the selected word or phrase and its book/chapter context, not pointer coordinates or unrelated reading text.
