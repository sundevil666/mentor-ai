...
function book(id: string, initialScore: number, rating?: 'very-hard' | 'hard' | 'comfortable' | 'easy'): PersonalReadingBook {
  return {
    ...
    difficultyAssessment: {
      version: 1, recommendation: rating === 'hard' || rating === 'very-hard' ? 'rewrite' : 'read', score: initialScore,
...
describe('reader book calibration', () => {
  it('lowers reader ability and raises a book when an easy-looking book proves hard', () => {
    ...
    assert.ok(prediction.calibratedDifficulty > 30);
    assert.equal(prediction.band, 'too-hard');
  });

  it('raises reader ability and lowers a book when a hard-looking book reads easily', () => {
    ...
    assert.ok(calibration.readerAbility > 45);
    assert.ok(prediction.calibratedDifficulty < 75);
